...
export default function Edit({ election }) {
  ...
  const { data, setData, put, errors } = useForm({
    ...
    level_name: electionData.level_name,
    start_on: electionData.election_start,
    end_on: electionData.election_end,
    country: electionData.level === "country" ? electionData.level_name : "",
    state: electionData.level === "state" ? electionData.level_name : "",
  });
  ...
  const onSubmit = (e) => {
    ...
    if (electionData.election_start !== data.start_on) {
      data.start_on = data.start_on.replace("T", " ") + ":00";
    }
    if (electionData.election_end !== data.end_on) {
      data.end_on = data.end_on.replace("T", " ") + ":00";
    }

    put(route("elections.update", electionData));
  };
